...
void arraylist_add(ArrayList* list, void* data) {
    if (list->size == list->capacity) {
        ...
        void* new_data = linear_allocator_alloc(list->allocator, new_capacity * list->element_size);
        if (!new_data) return;
        memcpy(new_data, list->data, list->size * list->element_size);
        list->data = new_data;
        list->capacity = new_capacity;
    }
    memcpy((char*)list->data + list->size * list->element_size, data, list->element_size);
    list->size++;
}
